...
namespace loader {
    ...
    struct InitTarget {
        bool dropDb;bool dropColl;bool stopBalancer;
        int chunksMin;
    };
...
//TODO: Sync delay, set on all replicas, then reset at end of load
    void setProgramOptions( Settings &settings, int argc, char *argv[] ) {
        ...
        ("shardKey,k", po::value<std::string>(&settings.shardKeyJson)->required(), "Dotted fields not supported (i.e. subdoc.field) must quote fields (\"_id\":\"hashed\"")
        ...
        if ( vm.count( "help" ) || !errormsg.empty() ) {
            ...
            if ( !errormsg.empty() ) std::cerr << "Unable to parse options: " + errormsg
                                               << std::endl;
            exit( 0 );
        }
        ...
        if ( initTarget.stopBalancer ) {
            ...
            assert( coll );
            if ( !mongoc_collection_update( coll,
