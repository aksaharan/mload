...
void setProgramOptions(Settings &settings, int argc, char *argv[]) {
    ...
    generic.add_options()
        ...
        ("writeConcern,w", po::value<int>(&settings.opAggSettings.writeConcern)->default_value(0), "write concern, # of nodes")
        ...
        ("batchSize,B", po::value<size_t>(&settings.opAggSettings.queueSize)->default_value(10000), "size of bson array sent to driver")
        ...
        ("opAggThreads", po::value<size_t>(&settings.opAggSettings.workThreads)->default_value(10), "Threads available to the Operations Aggregator to do work (i.e. spill to disk)")
        ("readQueueSize", po::value<long unsigned int>(&settings.queueSettings.queueSize)->default_value(10000), "Read queue size")
    ...
    if(vm.count("help") || !errormsg.empty()) {
        ...
        if(!errormsg.empty())
            std::cerr << "Unable to parse options: " + errormsg << std::endl;
        exit(0);
    }

    settings.process();
    if(settings.endPointSettings.directLoad)
        initTarget.stopBalancer = true;

    if(!boost::starts_with(settings.connection, "mongodb://"))
        settings.connection.insert(0, "mongodb://");
    //TODO: verify JSON
    ...
    if(settings.threads == 0)
        ...
        settings.threads = std::thread::hardware_concurrency() + settings.threads;

    if(settings.chunksPerShard <= 0 && settings.hashed)
        settings.chunksPerShard = 2;

    if(initTarget.chunksMin < 0)
        initTarget.chunksMin = settings.chunksPerShard;


    if(!is_directory(boost::filesystem::path(settings.loadDir))) {
        std::cerr << "loadPath is required to be a directory. loadPath: " << settings.loadDir << std::endl;
        exit(1);
    }
    ...
    if(initTarget.stopBalancer) {
        ...
        assert(coll);
        if(!mongoc_collection_update(coll, MONGOC_UPDATE_UPSERT, query, updater, NULL, &error)) {
    ...
    if(initTarget.dropColl || initTarget.dropDb || settings.shard()) {
        ...
        if(initTarget.dropDb) {
            ...
            if(!mongoc_client_command_simple(client, settings.database.c_str(), bsonCmd, NULL, NULL, &error))
                std::cerr << "Error dropping database (does it exist?): " << error.message << std::endl;
            ...
            if(!mongoc_client_command_simple(client, settings.database.c_str(), bsonCmd, NULL, NULL, &error))
                std::cerr << "Error dropping collection (does it exist?): " << error.message << std::endl;
            bson_destroy(bsonCmd);
        }
        ...
        if(settings.shard()) {
            if(!settings.shardKeyJson.empty()) {
                ...
                if(!mongoc_client_command_simple(client, "admin", bsonCmd, NULL, NULL, &error)) {
                ...
                bson_t *key = bson_new_from_json(reinterpret_cast<const uint8_t*>(settings.shardKeyJson.c_str()), settings.shardKeyJson.size(), &error);
                if(!key) {
                    std::cerr << "Error converting key(" << settings.shardKeyJson << ") to bson: " << error.message << std::endl;
                    exit(1);
                }
                if(settings.chunksPerShard > 0 && settings.hashed)
                    bsonCmd = BCON_NEW("shardCollection", (settings.database + "." + settings.collection).c_str(), "key", BCON_DOCUMENT(key), "numInitialChunks", BCON_INT32(settings.chunksPerShard * settings.shards.size()));
                else {
                    bsonCmd = BCON_NEW("shardCollection", (settings.database + "." + settings.collection).c_str(), "key", BCON_DOCUMENT(key));
                    //TODO: the presplits
                }
                if(!mongoc_client_command_simple(client, "admin", bsonCmd, NULL, NULL, &error)) {
                    ...
                    if(initTarget.dropColl) {
                        std::cerr << "Error sharding a collection that should have been dropped.  Exiting." << std::endl;
                        exit(1);
                    }
                }
                bson_destroy(bsonCmd);
    ...
    if(initTarget.stopBalancer) {
        ...
        mongoc_cursor_t *cursor;
        for(;;) {
            cursor = mongoc_collection_find(coll, MONGOC_QUERY_NONE, 0, 0, 1, selector, fields, NULL);
            if(!cursor) {
            ...
            if(mongoc_cursor_next(cursor, &balancerState)) {
                if(!mongoc_matcher_match(balancerRunning, balancerState))
                    break;
            }
    ...
    //if the setup is hashed, wait for chunks to disribute
    if(initTarget.chunksMin > 0 && settings.hashed) {
        ...
        bson_t *pipeline = BCON_NEW("pipeline", "[",
             "{", "$match", "{", "ns", settings.ns().c_str(), "}", "}",
             "{", "$group", "{", "_id", "$shard", "chunkCount", "{", "$sum", BCON_INT32(1), "}", "}", "}",
        "]");
        mongoc_cursor_t *cursor;
        //TODO: fix this when matcher is working
        bson_t *matchExpr = BCON_NEW("chunkCount", "{", "$gt", BCON_INT32(initTarget.chunksMin - 1), "}");
        ...
        for(;;) {
            ...
            while(mongoc_cursor_next(cursor, &doc)) {
                if(!mongoc_matcher_match(minChunk, doc))
                    ++fail;
            }
            if(mongoc_cursor_error(cursor, &error)) {
                std::cerr << "Error checking chunks aggregation cursor failed: " << error.message << std::endl;
                exit(1);
            }
            if(fail == 0) break;
            if(!loopCount) {
                std::cout << "Waiting for chunks to balance (min " << initTarget.chunksMin << " chunks per shard)";
            }
            mongoc_cursor_destroy(cursor);
            if(!(++loopCount % 60))
                std::cout << fail << " shard(s) behind (manual intervention required for balancing to complete?)" << std::flush;
            else
                std::cout << "." << std::flush;
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
